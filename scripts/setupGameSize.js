...
commander
    ...
    .option('--scale-mode <val>', 'Scale mode of your game (Default: ' + defaultScaleMode + ') [USER_SCALE, SHOW_ALL, EXACT_FIT, NO_SCALE, RESIZE]', function (val) {
...
if (scaleMode === 'USER_SCALE') {
    ...
    var isPortrait = baseWidth < baseHeight;
    ...
    if (isPortrait) {
        maxWidth = Math.round(768 * (baseHeight / 1024));
    }
    else {
    ...
    if (isPortrait) {
        maxHeight = Math.round(1136 * (baseWidth / 640));
    }
    else {
